...
module.exports = (bot, { viewDistance = 6, firstPerson = false, port = 3000, prefix = '' }) => {
  ...
  bot.viewer.drawBox = (id, start, end, color = 0x00ff00) => {
    primitives[id] = { type: 'box', id, start, end, color }
    for (const socket of sockets) {
      socket.emit('primitive', primitives[id])
    }
  }

  bot.viewer.drawSphere = (id, position, radius = 0.5, color = 0xff00ff) => {
    primitives[id] = { type: 'sphere', id, position, radius, color }
    for (const socket of sockets) {
      socket.emit('primitive', primitives[id])
    }
  }

  bot.viewer.drawText = (id, position, text, color = '#ffffff') => {
    primitives[id] = { type: 'text', id, position, text, color }
    for (const socket of sockets) {
      socket.emit('primitive', primitives[id])
    }
  }
